...
export default function Navbar() {
	...
	return (
		<nav>
			...
			<div className="hidden lg:flex lg:border justify-center items-between py-3 bg-white/70 backdrop-blur-sm rounded-xl fixed top-4 left-1/2 -translate-x-1/2 border border-gray-200 z-50 px-5 pl-3">
				{/* Logo */}
			...
			<div
				className={`lg:hidden fixed bottom-4 left-1/2 -translate-x-1/2 bg-white/70 backdrop-blur-sm rounded-xl p-2 flex items-center justify-between z-50 border border-gray-200 w-fit px-2 transition-all duration-300 ${
			...
			>
				...
				{/* Menu/X Icon */}
				<button
					className="ml-3 p-2 rounded-[7px] hover:bg-gray-100 transition-all duration-300 bg-gray-100 border border-gray-300 flex items-center justify-center md:p-3"
			...
			>
				<div className="relative w-fit max-w-xs sm:max-w-sm md:max-w-[40vw] lg:max-w-[32vw] xl:max-w-[28vw] 2xl:max-w-[24vw] rounded-xl border border-blue-100 p-4 flex flex-col items-center shadow-sm bg-white/70 backdrop-blur-sm ">
					{navItems.map((item) => (
						<a
							key={item.label}
							href={item.href}
