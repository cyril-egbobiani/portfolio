...
export default function Navbar() {
	...
	return (
		<nav>
			...
			{/* Desktop Let's Talk Button - Only shows after scrolling past header */}
			<a
				href="#contact"
				className={`hidden lg:fixed lg:top-4 lg:right-4 px-6 py-2 rounded-full bg-black text-white z-50 transition-all duration-700
    ${showTalkButton ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8 pointer-events-none"}
  `}
				style={{
					boxShadow:
			...
			>
				...
				<a
					href="#contact"
					className="mr-2 p-2 rounded-full bg-black/85 flex items-center justify-center"
					style={{
						boxShadow:
							"inset 4px -2px 5px -2px #96968b, inset -3px 2px 5px -2px #e6e6e6, 0 8px 30px rgba(123,92,250,0.2)",
					}}
				...
				{/* Menu/X Icon */}
				<button
					className="ml-2 p-2 rounded-3xl transition-all duration-300 bg-black/85 flex items-center justify-center"
			...
			>
				<div
					...
					className="relative  w-fit max-w-xs sm:max-w-sm md:max-w-[40vw] lg:max-w-[32vw] xl:max-w-[28vw] 2xl:max-w-[24vw] bg-[#ffffff63] p-4 flex flex-col items-end shadow-sm rounded-4xl backdrop-blur-2xl"
				>
					{navItems.map((item) => (
						<a
							key={item.label}
